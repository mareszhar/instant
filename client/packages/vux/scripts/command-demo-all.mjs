...
function runCommandForDemo(command, demoRoot) {
  console.log(`[demo-command:${command}] running in ${path.relative(VUE_ROOT, demoRoot)}`)
  ...
  throw new Error(
    `${command} failed in ${path.relative(VUE_ROOT, demoRoot)} (exit ${result.status ?? 'unknown'})`,
  )
}
