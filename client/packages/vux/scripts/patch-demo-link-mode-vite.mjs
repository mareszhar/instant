 ...
 *   - `vite.optimizeDeps.exclude` contains `@mszr/idb-vux`
 * - non-`links` modes remove only those link-mode additions
 *
 * Safety/cleanup rules:
 * - only removes owned keys (`preserveSymlinks`, `@mszr/idb-vux` exclude entry)
 ...
 */
const LINK_MODE_OPTIMIZE_DEPS_EXCLUDES = ['@mszr/idb-vux']
