...
13. [Custom Matchers — ts-probe/vitest](#13-custom-matchers--ts-probevitest)
...
```
ts-probe              # core, runner-agnostic
ts-probe/vitest       # optional: extends global expect with ts-probe matchers
...
interface DefineProjectConfig {
  /**
   * Path to a tsconfig.json. If omitted, ts-probe auto-detects the nearest tsconfig.json
   * walking up from the test file. If none is found, a strict in-memory TypeScript context
   * is used.
   */
  tsconfig?: string
...
describe('my suite', () => {
  ...
  it('a test', () => {
    // ✗ test scope — invalid, ts-probe logs a warning
...
### Single-cursor result

Returned by `project.query` when the template contains exactly one `cursor` (named or unnamed).
...
interface GroupQueryResult {
  ...
  for: (apiName: string) => SingleCursorResult | MultiCursorResult
...
## 13. Custom Matchers — ts-probe/vitest
...
| Situation | Behavior |
|---|---|
| Cursor in invalid position (e.g., mid-token) | `completions = []`, `hover = null`, all fields empty/null |
...
Exposes the raw TypeScript `LanguageService` instance for cases the ts-probe API does not cover:
...
*Outcome: a single-cursor query that returns completions and errors works end-to-end.*

- TypeScript Language Service wrapper class
...
- First integration test suite passing

### Phase 2 — Full Result Surface

*Outcome: all result fields are populated; named cursors work; rich completion access works.*
...
*Outcome: snippets and groups are defined and composable; cursor namespacing works correctly.*
...
- Nested snippet cursor namespacing — full path resolution
- `group([...])` and `group('label', [...])` primitives
- Validation: unnamed snippet in multi-use position produces clear error

### Phase 4 — Group Queries & Parity

*Outcome: `project.queryGroup` works; `hasParity` and `divergence` are correct.*
...
### Phase 5 — Project Flexibility

*Outcome: aliases, scoped overrides, and mode matrix all work.*

- `aliases` option in `defineProject`
- `project.with({ aliases?, files? })` — incremental clone with merge behavior
- `project.forModes(modes, callback)` — string and object config shorthand
- `project.entry` on scoped mode project
- Lazy program construction per mode

### Phase 6 — Matcher Addon

*Outcome: `ts-probe/vitest` is importable; all matchers work; type snapshots work.*

- `ts-probe/vitest` entry point — side-effect import pattern
...
- `toMatchTypeSnapshot()` — separate snapshot bucket, Vitest lifecycle integration
- `ts-probe/jest` — parallel implementation for Jest

### Phase 7 — Polish & Escape Hatches

*Outcome: the library is production-ready; escape hatches are documented and usable.*

- `project.languageService` escape hatch
- `project.virtualPath` and `project.positionOf()` helpers for raw LS access
...
- README, API reference documentation
- Performance benchmarks for typical test suite sizes
  ...
  project.forModes(
    {
      'source':   './src/index.ts',
      'dist-esm': { entry: './dist/esm/index.js',       dts: './dist/index.d.ts' },
      'dist-cjs': { entry: './dist/commonjs/index.js',  dts: './dist/index.d.ts' },
    },
    (project, mode) => {
