...
>(
  ...
  watchEffect((onCleanup) => {
    // DUX-DELTA(ssr): no topic subscription on the server.
    if (!isClient())
      return
    // END DUX-DELTA
...
>(
  ...
  watchEffect((onCleanup) => {
    // DUX-DELTA(ssr): no room join on the server.
    if (!isClient())
      return
    // END DUX-DELTA
...
>(
  ...
  watchEffect((onCleanup) => {
    // DUX-DELTA(ssr): no presence subscription on the server.
    if (!isClient())
      return
    // END DUX-DELTA
    const type = toValue(room.type)
...
>(
  ...
  watchEffect(() => {
    // DUX-DELTA(ssr): no presence publish on the server.
    if (!isClient())
      return
    // END DUX-DELTA
    room.core._reactor.publishPresence(
