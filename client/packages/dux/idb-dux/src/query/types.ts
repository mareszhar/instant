 ...
 * field doesn't support are typed as the `IDBDUXERR_*` message explaining why.
 */
...
type ComparableValue<V, IsIndexed, Op extends string> = [V] extends [
  ...
  ? IsIndexed extends true
    ? WireValue<V>
    : `IDBDUXERR_WHERE_OPERATOR_INVALID: Operator ${Op} is only available for indexed fields with a checked type.`
  : `IDBDUXERR_WHERE_OPERATOR_INVALID: Operator ${Op} is only available for indexed fields with a checked type.`

type StringPattern<V, IsIndexed, Op extends string> = [V] extends [string]
  ? IsIndexed extends true
    ? string
    : `IDBDUXERR_WHERE_OPERATOR_INVALID: Operator ${Op} is only available for indexed string fields.`
  : `IDBDUXERR_WHERE_OPERATOR_INVALID: Operator ${Op} is only available for indexed string fields.`
