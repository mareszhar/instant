...
The contract per level, with the intended diagnostic. Diagnostics carry stable `IDBDUXERR_*` codes so editor-DX tests can assert messages, not just positions.

**Namespace level** — top-level query keys are limited to schema namespace names.

> `IDBDUXERR_QUERY_ROOT_KEY_UNKNOWN: foo is not a valid top-level namespace`
...
> `IDBDUXERR_QUERY_NESTED_KEY_UNKNOWN: foo is not a valid nested key on tasks`
...
> `IDBDUXERR_WHERE_KEY_UNKNOWN: tagName is not a valid where key on tasks`
...
**Result-key collision level** — a top-level scope whose *resolved* key (or top-level `$m` label) hits a reserved result field is flagged with `IDBDUXERR_RESULT_KEY_RESERVED`. The full contract — the reserved set, why it's caught here and not at `defineSchema` — is [§4.5](#45-result-key-collisions).
...
> `IDBDUXERR_RESULT_KEY_RESERVED: result key 'state' is reserved — it clashes with a hook result field; rename this scope`
...
src/
  ...
  query/
    ...
    validation.ts   # the one validation surface — IdbValidQuery + IDBDUXERR_* arm
...
- **Validation** is `IdbValidQuery<Q, S>` — an intersection of the input-independent authoring shape `IdbQuery<S>` (the completion source) and an *input-mapped arm* that re-walks the user's own keys and types each violation as a stable `IDBDUXERR_*` message string at the offending key (valid positions resolve to `unknown`, the intersection identity). `q`/`defineQuery` apply it as a `<const Q extends IdbValidQuery<Q, S>>` self-referential constraint; inline literals get it as a contextual type, and wrapping a factory's return in `q()` restores the same deep localization. (Operator restrictions live in the authoring `IdbWhereOps` shape, typed as their `IDBDUXERR_*` message when the field doesn't support the operator.)
...
- `q({ states: { $: { $only } } })` → `IDBDUXERR_RESULT_KEY_RESERVED` on the colliding scope
...
- each `IDBDUXERR_*` diagnostic, asserted by message at its cursor
...
- [x] validation types: root keys, 3-hop traversal, where keys/dot-paths (fields + links, 3 hops, link-terminating), operator table, order rules, result-key collisions (`IDBDUXERR_RESULT_KEY_RESERVED`), `IDBDUXERR_*` messages
...
- [x] `.dx.test.ts`: every validation level + every `IDBDUXERR_*` message at its cursor; inline vs factory localization both locked
