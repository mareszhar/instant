 ...
 * - supports main demo (`idb-dux/demo`) and sandbox demos (`sandbox/*`)
 *
 * Out of scope:
...
const PACKS_DIR = path.resolve(WORKSPACE_ROOT, 'packs')
...
const RESOLUTION_RULES = {
  '@mszr/idb-dux': {
    tarballPattern: /^mszr-idb-dux-.*\.tgz$/,
    linkSpec: 'link:@mszr/idb-dux',
    npmSpec: 'npm:@mszr/idb-dux@latest',
  },
  '@instantdb/admin': {
...
function applyModeToDemoManifest(demoRoot, mode) {
  ...
  if (mode !== MODE.TARBALLS && hadFileBasedEntries && fs.existsSync(lockPath)) {
    ...
    console.log(`[demo-idb-resolution] removed bun.lock: ${path.relative(WORKSPACE_ROOT, lockPath)}`)
  }

  if (changes.length === 0) {
    console.log(`[demo-idb-resolution] already in '${mode}' mode: ${path.relative(WORKSPACE_ROOT, manifestPath)}`)
    return
  }
  ...
  for (const change of changes) {
    console.log(
      `[demo-idb-resolution] ${path.relative(WORKSPACE_ROOT, manifestPath)} ${change.field}.${change.depName}: ${change.previous} -> ${change.next}`,
    )
  }

  console.log(`[demo-idb-resolution] updated ${changes.length} entr${changes.length === 1 ? 'y' : 'ies'} for ${path.relative(WORKSPACE_ROOT, demoRoot)}`)
}
...
function printDemoStatus(demoRoot) {
  ...
  console.log(`[demo-idb-resolution] manifest: ${path.relative(WORKSPACE_ROOT, manifestPath)}`)
