...
function getTargetDemoRoots(scope, pickedDemoName) {
  ...
  if (scope === 'pick') {
    if (typeof pickedDemoName !== 'string' || !pickedDemoName.trim()) {
      throw new Error('Missing --demo for scope "pick". Example: --demo main or --demo demo-ssr-bug')
    }
...
function pickWritableTempDir() {
  const candidates = [
    process.env.TMPDIR,
    '/private/tmp',
    path.resolve(WORKSPACE_ROOT, '.tmp'),
  ].filter(Boolean)
...
function refreshDemo(
  ...
  console.log(`[demo-refresh] clearing runtime caches: ${path.relative(WORKSPACE_ROOT, demoRoot)}`)
  ...
  if (options.prep) {
    console.log(`[demo-refresh] regenerating Nuxt prep artifacts: ${path.relative(WORKSPACE_ROOT, demoRoot)}`)
