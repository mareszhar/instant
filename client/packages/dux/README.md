...
1. `pnpm run workspace:setup` — install everything needed to work on dux
2. `pnpm run sdk:build:all` — build the official workspace deps, then ours
3. `pnpm run sdk:build:ours` — build `@mszr/idb-dux` (all entrypoints)
4. `pnpm run sdk:test` — run every assertion plane (runtime, types, editor DX)
5. `pnpm run sdk:typecheck`
6. `pnpm run sdk:lint` / `pnpm run lint`
